...
Manages DeepSeek large language models.
"""
...
class DeepSeekLLMModelManager:
    """
    Manages configuration and initialization of DeepSeek LLM models.
    
    This class:
    1. Tracks available DeepSeek models
    ...
    AVAILABLE_MODELS = {
        'deepseek-ai/deepseek-coder-33b-instruct': {
            'description': 'DeepSeek Coder 33B - large model specialized for code generation',
            ...
            'capabilities': ['coding', 'reasoning', 'document_processing']
        },
        'deepseek-ai/deepseek-v3-7b': {
            'description': 'DeepSeek v3 7B - high quality reasoning and instruction following',
            'model_type': 'AutoModelForCausalLM',
            'tokenizer_type': 'AutoTokenizer',
            'context_length': 16384,
            'requirements': {
        ...
        'deepseek-ai/deepseek-llm-67b-chat': {
            'description': 'DeepSeek LLM 67B Chat - large high-performance chat model',
            'model_type': 'AutoModelForCausalLM',
            'tokenizer_type': 'AutoTokenizer',
            'context_length': 8192,
            'requirements': {
    ...
    def __init__(self, config: Dict[str, Any] = None):
        """
        Initialize the DeepSeek LLM model manager.
        
        Args:
            config: Configuration for the model manager
        """
        self.config = config or {}
        self.model_name = self.config.get('model', 'deepseek-ai/deepseek-v3-7b')
        
        # Model configuration
        self.model_config = self.AVAILABLE_MODELS.get(self.model_name, self.AVAILABLE_MODELS['deepseek-ai/deepseek-v3-7b'])
    ...
    def is_available(self) -> bool:
        """
        Check if the DeepSeek model can be loaded on the current device.
        ...
        if self.model_config.get('requirements', {}).get('gpu_memory', 0) > 0 and self.device == 'cpu':
            logger.warning(f"DeepSeek model {self.model_name} requires GPU but none is available")
            return False
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            logger.info(f"DeepSeek model {self.model_name} loaded successfully")
    ...
    def list_available_models(self) -> List[Dict[str, Any]]:
        """
        List all available DeepSeek models with their configurations.
    ...
    def prepare_generation_parameters(self, prompt: str, system_message: Optional[str] = None) -> Dict[str, Any]:
        """
        Prepare generation parameters for the DeepSeek model.
        ...
        if system_message:
            if hasattr(self.tokenizer, 'apply_chat_template'):
                ...
                # Fallback for DeepSeek chat format
                if "coder" in self.model_name.lower():
