...
Provides text generation services using Qwen and QwQ large language models.
"""
...
from .model_manager import QwenLLMModelManager
...
class QwenLLMService(BaseLLMProvider):
    """
    Service for text generation with Qwen and QwQ models.
    
    This service provides:
    1. Text generation with Qwen models
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.model_manager = QwenLLMModelManager(config)
        self.system_message = config.get('system_message', 'You are a helpful AI assistant.')
        self.model_name = config.get('model_name', 'qwen2-7b')
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.model_manager.is_available():
                logger.error("Qwen service is not available on this device")
                raise ModelUnavailableError(
                    "Qwen model is not available on this device", 
            ...
            logger.error(f"Error initializing Qwen service: {str(e)}")
            raise ProviderConnectionError(
                f"Failed to initialize Qwen service: {str(e)}", 
                provider="qwen",
                model=self.model_name
            )
    ...
    def generate_text(self, prompt: str, max_tokens: Optional[int] = None) -> Tuple[str, float]:
        ...
        try:
            ...
            # Log timing
            logger.info(f"Qwen model generated {len(output_text)} characters in {generation_time_ms/1000:.2f} seconds")
            ...
            if self.metrics:
                ...
                context_window_size = getattr(self.model_manager, 'context_window', 4096)  # Default for Qwen
    ...
    def generate_batch(self, prompts: List[str], max_tokens: Optional[int] = None) -> List[Tuple[str, float]]:
        ...
        try:
            ...
            logger.error(f"Error generating batch with Qwen model: {str(e)}")
            raise
        
    @handle_llm_errors
    def process_document(self, document: Dict[str, Any], query: Optional[str] = None) -> Dict[str, Any]:
        """
        Process a document with the Qwen model.
        ...
        try:
            ...
            logger.error(f"Error processing document with Qwen model: {str(e)}")
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.error(f"Error during cleanup of Qwen model: {str(e)}") 